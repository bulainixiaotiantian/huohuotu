...
exports.delRole = async function(request,reply){
    ...
    if(sysCount && sysCount!=0){
        reply({"message":"删除角色失败，该角色下有管理员，请先将管理员设置为其他角色","statusCode":104,"status":false});
    };

    var userCount = await dao.findCount(request,"user",{"roleId":new ObjectID(request.params.roleId)});
    if(userCount && userCount!=0){
        reply({"message":"删除角色失败，该角色下有用户，请先将用户设置为其他角色","statusCode":104,"status":false});
    };
    ...
    db.collection('role').deleteOne({"_id":new ObjectID(request.params.roleId)},function(err,result){
        if(err){
            ...
            throw err;
            reply({"message":"删除角色失败","statusCode":104,"status":false});
        }else{
            reply({"message":"删除角色成功","statusCode":103,"status":true});
        }
    });
